				...
				<div class="single-product-slider">
					<div id='carousel-custom' class='carousel slide' data-ride='carousel'>
							...
							<div class='carousel-inner'>
                                @foreach ($getImages as $key => $item)
                                    <div class='item {{ $key==0 ? 'active' : '' }}'>
                                        <img src='{{ asset('images/product').'/'.$item->images }}' alt='' data-zoom-image="{{ asset('images/product').'/'.$item->images }}" />
                                    </div>
                                @endforeach
							</div>
							<!-- sag sol -->
						...
						<ol class='carousel-indicators mCustomScrollbar meartlab'>
                            @foreach ($getImages as $key => $item)
                                <li data-target='#carousel-custom' data-slide-to='0' class='{{ $key==0 ? 'active' : '' }}'>
...
<section class="products related-products section">
	<div class="container">
		...
		<div class="row">
            ...
            @php
                $getImage = App\Models\ProductImage::where('product_id', $product->id)->first();
            @endphp
			<div class="col-md-3">
