        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" id="custom_form" action="{{ route('products.update', $product->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('patch')
                            ...
                            <div class="col-sm-10">
                                <select name="category_id" id="category_id" class="form-control p_input @error('category_id') is-invalid @enderror" autofocus>
                                    ...
                                    @foreach ($categories as $value)
                                        <option value="{{$value->id}}" {{ old('category_id', $product->category_id) == $value->id ? 'selected' : '' }}>{{ucfirst($value->name)}}</option>
                                    @endforeach
                                </select>
                                @error('category_id')
                            ...
                            <div class="col-sm-10">
                                <select name="sub_category_id" id="sub_category_id" class="form-control p_input @error('sub_category_id') is-invalid @enderror" autofocus>
                                    ...
                                    @foreach ($sub_categories as $value)
                                        <option value="{{$value->id}}" {{ old('sub_category_id', $product->sub_category_id) == $value->id ? 'selected' : '' }}>{{ucfirst($value->name)}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input id="quantity" type="text" class="form-control @error('quantity') is-invalid @enderror" name="quantity" value="{{ old('quantity', $product->quantity) }}" autocomplete="quantity">
                                @error('quantity')
                                    <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input id="description" type="text" class="form-control @error('description') is-invalid @enderror" name="description" value="{{ old('description', $product->description) }}" autocomplete="description">
                                @error('description')
                                    <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                @enderror
                            </div>
